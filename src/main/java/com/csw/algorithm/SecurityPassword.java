...
public class SecurityPassword {
    ...
    public static void main(String[] args) {
        ...
        //生成个数
        int numBer = 11;
        dxsz(maxLength, numBer);//大写+小写+字母+特殊符号
    ...
    private static void dxsz(int maxLength, int numBer) {
        ...
        for (int n = 0; n < numBer; n++) {
            StringBuilder base = new StringBuilder(a1[(int) (Math.random() * (a1.length))] + a2[(int) (Math.random() * (a2.length))] +
                    ...
                    a5[(int) (Math.random() * (a5.length))] + a6[(int) (Math.random() * (a6.length))]);
            for (int i = 0; i < maxLength - MIN_LENGTH; i++) {//还有多少个字母
                int indexA = (int) (Math.random() * 6 + 1);
                if (indexA == 1) {
        ...
        System.out.println("按照每秒2000次破解速度破解需要" + (long) (Math.pow((a1.length + a2.length + a3.length + a4.length + a5.length + a6.length), maxLength) / 365 / 24 / 60 / 60 / 2000) + "年");
    }
}
